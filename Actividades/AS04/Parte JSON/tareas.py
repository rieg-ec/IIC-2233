...
def cargar_ayudantes(ruta):
    ...
    with open('tareas.json', 'r') as file:
        ...
        for key in archivo:
            usuario = desencriptar(key)
            cargo, pok, pizza, serie = map(lambda x: desencriptar(x),
                                            (value for value in archivo[key]))
            ayudante = AyudanteTareo(usuario, cargo, pok, pizza, serie)
