...
class Servidor:
        ...
        while True:
            ...
            if socket_cliente:
                ...
                thread = threading.Thread(
                    ...
                    daemon=True, args=(socket_cliente, ))
                thread.start()
                break
    ...
    def escuchar_cliente(self, socket_cliente, id_cliente=0):
        ...
        try:
            # =========================== COMPLETAR ===========================
            while True:
                response_length_bytes = socket_cliente.recv(5)
                response_length = int.from_bytes(
                    response_length_bytes, byteorder='little')
                response = bytearray()
                while len(response) < response_length:
                    read_length = min(128, response_length - len(response))
                    response.extend(socket_cliente.recv(read_length))

                received = response.decode()
                if received != '':
                    dict_response = json.loads(received)
                    print(dict_response)
    ...
    def enviar(self, socket_cliente, mensaje):
        ...
        largo_mensaje_bytes = largo_mensaje_codificado.to_bytes(
                5, byteorder='little')
        socket_cliente.sendall(largo_mensaje_bytes + mensaje_codificado)
