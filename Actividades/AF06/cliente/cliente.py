...
class Cliente(QObject):
        ...
        try:
            while self.conectado:
                ...
                largo_mensaje = int.from_bytes(
                    largo_mensaje_bytes, byteorder='little')
                respuesta = bytearray()
                while len(respuesta) < largo_mensaje:
                    largo_lectura = min(128, largo_mensaje - len(respuesta))
                    respuesta.extend(self.socket_cliente.recv(largo_lectura))

                recibido = respuesta.decode()
                if recibido != '':
                    recibido_dict = json.loads(recibido)
                    self.senal_a_interfaz.emit(recibido_dict)
    ...
    def enviar(self, mensaje):
        ...
        mensaje_codificado = mensaje_serializado.encode()
        largo_mensaje_codificado = len(mensaje_codificado)
        largo_mensaje_bytes = largo_mensaje_codificado.to_bytes(
            5, byteorder='little')
        self.socket_cliente.sendall(largo_mensaje_bytes + mensaje_codificado)
